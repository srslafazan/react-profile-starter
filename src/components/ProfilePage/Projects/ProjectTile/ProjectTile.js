...
// 		return (
			...
			// Do this after making projectList
...
class ProjectTile extends React.Component {
	render() {
		var projectNodes = this.props.data.map((project) => {
			return (
				<Project name={project.projName} key={project.id}>
					//Project content
					<img className={s.projectThumb} src={project.projThumbnail} />
					<h1>{project.projName}</h1>
					<p>{project.projDescription}</p>
    			<Button>Check it out!</Button>
    			<Row>
    				<Col s={3} m={3} l={3}>
    					// testimonial author's image
		    			<img className={s.testmonialImg} src={project.testimonialAuthorImg} />
	    			</Col>
	    			<Col s={9} m={9} l={9}>
	    				// testimonial text
		    			<p className={s.testimonialText}>{project.testimonial}</p>
		    			// testimonial author name
		    			<p>{project.testimonialAuthor}</p>
	    			</Col>
    			</Row>
				</Project>
			);
